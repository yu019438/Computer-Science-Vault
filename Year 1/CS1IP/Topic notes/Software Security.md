...
- All of the above are examples of an adversarial input attack
...
```
- This input forces the algorithm to traverse an immensely large linked list before finding the target -> exploiting the complexity of the `collections()` method
### Buffer overflow attack
- A buffer overflow attacks is when a threat actor writes a program that assigns more memory to the buffer than it can contain at any one time, causing an overflow of data into the return address
...
```Java
void processInput(String advInputm int amountToCopy) {
